...
let handler = async (m, { conn, args, usedPrefix, DevMode }) => {
    if (args.length < 3) {
            ...
            case 'balance':
                if (global.db.data.users[m.sender].balance >= count * 1) {
                    try {
                        ...
                        conn.reply(m.chat, `Berhasil mentransfer balance sebesar ${count}`.trim(), m)
                        conn.sendFile(m.chat, 'https://telegra.ph/file/5e043d7cd784d2bfa5b67.jpg', caption, m)
                    } catch (e) {
                        global.db.data.users[m.sender].balance += count * 1
        ...
        conn.reply(m.chat, `Gunakan format ${usedPrefix}transfer <type> <jumlah> <@tag>\📍 Contoh penggunaan: *${usedPrefix}transfer balance 100 @tag*\n\n*List yang bisa di transfer :*\n💹 balance\n💳 Tabungan\n🥤 Potion\n🗑️ Sampah\n💎 Diamond\n📦 Common\n🛍️ Uncommon\n🎁 Mythic\n🧰 Legendary\n🕸️ String\n🪵 Kayu\n🪨 Batu\n⛓ iron`.trim(), m)
        console.log(e)
        if (DevMode) {
            for (let jid of global.owner.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').filter(v => v != conn.user.jid)) {
                conn.sendMessage(jid, 'Transfer.js error\nNo: *' + m.sender.split`@`[0] + '*\nCommand: *' + m.text + '*\n\n*' + e + '*', MessageType.text)
