			...
			for (let name in global.plugins) {
				...
				if ((usedPrefix = (match[0] || '')[0])) {
					...
					if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
						this.reply(m.chat, `🐱 Your limit runs out, please buy through ${usedPrefix}buylimit, or contact the owner Kemii Denpai +6288980870067`, m)
						continue
					}
...
global.dfail = (type, m, conn) => {
    let tdan = 'https://telegra.ph/file/0b32e0a0bb3b81fef9838.jpg'
    let user = global.db.data.users[m.sender]
	let msg = {
