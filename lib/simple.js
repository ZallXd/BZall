...
`.trim()
    ...
    */
    ...
    conn.sendButtonDoc = async (jid, content, footerText, button1, id1, quoted, options) => {
      ...
      const buttonMessage = {
        document: { url: "https://wa.me/+6288980870067" },
...
/*exports.smsg = (conn, m, hasParent) => {
    ...
    m.reply = async (text, chatId, options) => {
    	let pp = await conn.profilePictureUrl(m.sender, 'image').catch(_ => 'https://telegra.ph/file/9a86c7e55316f79ea4527.jpg')
        let { data } = await conn.getFile(await(await require('node-fetch')(pp)).buffer())
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { title: 'Arsss GANTENG : 3 - Sooo Coolll', body: 'Simple Bot Menu By αяzs αρα cσвα', sourceUrl: 'http://wa.me/62882003146309', thumbnail: data }}, options })
    }
    m.name = m.pushName || conn.getName(m.sender)
